/*************************************************************************
  ...
  This file tests out the lease control mechnanisms written inside the posix
  interface.
************************************************************************/
...
READ_START:
  ...
  printf("Throughput: %3.3f MB\n",
         (float)((file_size_bytes) >> 20) / (float)time_stats_get_avg(&stats));
...
long int calculate_time(struct timeval *t_before, struct timeval *t_after) {
  if (t_after->tv_usec < t_before->tv_usec) {
    t_after->tv_usec += 1000000;
    t_after->tv_sec -= 1;
  }
  return (t_after->tv_sec - t_before->tv_sec) * 1000000 +
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < num_processes; ++i) {
    time.emplace_back(
        make_pair((struct timeval *)malloc(sizeof(struct timeval)),
  ...
  while (num_processes > 0) {
    ...
    --num_processes; // TODO(pts): Remove pid from the pids array.
#ifdef MLFS
    shutdown_fs();
#endif
    fflush(stdout);
