...
typedef enum {FS, NVM, SPDK, DRAM} test_mode_t;
...
int spdk_init(uint8_t dev, ssize_t map_size)
{
    spdk_async_io_init();
}

int spdk_read(uint8_t dev, uint64_t *buf, addr_t blockno, uint32_t io_size)
{
	return spdk_async_io_read((unsigned char *)buf, blockno, io_size,
			NULL, NULL);
}

int spdk_write(uint8_t dev, uint64_t *buf, addr_t blockno, uint32_t io_size)
{
...
ssize_t mem_bench::io_read(int fd, void *buf, size_t count)
{
	...
	switch (test_mode) {
		...
		case NVM: 
			...
			break;
		case SPDK: 
			ret = spdk_read(0, (uint64_t *)buf, offset >> block_size_shift , count);
			if (ret > 0)
				offset += ret;
			count = ret;
			break;
	...
	switch (test_mode) {
		...
		case SPDK:
		  //we need the return value of the call
			ret  = spdk_write(0, (uint64_t *)buf, offset >> block_size_shift, (uint32_t)count);
			if (ret > 0)
				offset += ret;
			count = ret;
			break;
...
void mem_bench::prepare(void)
{
	...
	if (test_mode == FS) {
		char file_path[256];
		unsigned int len = sprintf(file_path,"%s/file0", test_dir_prefix);
...
#if 1
	if (test_mode == FS || test_mode == SPDK) {
...
{
	std::cerr << "usage: " << prog  
		<< " <wr/sr/sw/rr/rw> <fs/nvm/spdk/dram>" 
